...
class MailMessage(models.Model):
            ...
            if (vals.get('model') == 'discuss.channel' 
                ...
                if (channel.exists() 
                    ...
                    if chatbot_script.title == "Gemini":
                        ...
                        # ✅ ADD CHECK HERE: Is this message from Gemini itself?
                        gemini_script = self.env['chatbot.script'].search([('title', '=', 'Gemini')], limit=1)
                        if gemini_script:
                            gemini_partner_id = gemini_script.operator_partner_id.id
                            
                            # If message author is Gemini, skip
                            if vals.get('author_id') == gemini_partner_id:
                                print("⏭️ Message is from Gemini itself, skipping...")
                                continue
    ...
    def _call_gemini(self, user_message):
        ...
        API_KEY = "AIzaSyBobw7I5PXGmM3Y7lxTlaoFRwYEIP_6Ozs"
